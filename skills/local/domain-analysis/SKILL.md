...
description: Analyze a business domain to identify entities, relationships, rules, and process flows. Use when modeling a new feature area, clarifying business requirements, or mapping existing processes before spec writing.
source: local
---

## Purpose

Produce a structured analysis of a business domain by identifying its key entities, relationships, business rules, and process flows. The output feeds `business.md` and provides the raw material for `/po` to structure into epics and user stories.

Use this skill when:
- A new feature area needs to be understood before spec writing
- Business requirements are vague or ambiguous
- Gaps exist between the described domain and existing specs in `docs/specs/`
...
1. **Read existing context**
   - Read `business.md` for product vision and existing domain knowledge
   - Read `architecture.md` for technical constraints
   - Read `docs/specs/` for existing functional scope
...
6. **Identify gaps**
   - Compare the domain model against existing specs in `docs/specs/`
...
7. **Propose output**
   ...
   - Suggest updates to `business.md` or new spec fragments for `/po`
