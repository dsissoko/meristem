...
- Post a comment in the issue explaining what is missing
- Ask targeted questions to obtain the missing information
- Once the user replies, update the file(s) with the new information
- Do NOT proceed to Gate 2 until both files are valid

**If both files are valid:** proceed to Gate 2.
---
...
- If they write "OK" or equivalent approval → proceed to Gate 4 with the proposed config
- If they propose changes → acknowledge, apply changes to the config, confirm in a reply,
...
```json
{
  ...
  "type": "scrum-init",
  "issue_number": 93,
  "project_name": "Journal Personnel",
  "mode": "A",
  "epics": [
    {
...
### Step 2 — Write and upload the manifest

Use your Bash tool to write the manifest and upload it as an artifact.
...
# Upload as GitHub Actions artifact (retention 1 day)
gh api repos/{REPO}/actions/runs/${GITHUB_RUN_ID}/artifacts \
  --method POST \
  --field name="manifest-${GITHUB_RUN_ID}" \
  --field retention_days=1
```

**Important:** use `actions/upload-artifact` mechanism — the post-run step in
`run-agent.yml` will download it via `actions/download-artifact@v4` using the same `run_id`.
...
- `project_name`: the name validated by the user in Gate 3
